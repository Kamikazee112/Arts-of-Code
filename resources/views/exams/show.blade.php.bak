...
@section('content')
    <div class="max-w-[800px] mx-auto">
        ...
        <div class="card p-6 mb-6">
            <div class="flex justify-between items-start mb-4">
                ...
                @if(auth()->check() && auth()->user()->role === 'admin')
                    <div class="action-group">
                        <a href="/exams/{{ $exam->id }}/edit" class="btn-sm btn-sm-edit" style="padding: 7px 14px; font-size: 13px;">
                            Edit
                        </a>
                    </div>
                @endif
            ...
            <div class="flex items-center gap-2">
                <span
                    class="badge {{ $exam->is_active ? 'bg-green-100 text-green-800' : 'bg-gray-100 text-gray-800' }} px-2 py-1 rounded text-xs">
                    ...
                    @foreach($exam->questions as $index => $question)
                        <div class="border-b border-[var(--border)] pb-4 last:border-0 last:pb-0">
                            <div class="flex items-start gap-3">
                                <div
                                    class="w-6 h-6 rounded-full bg-[var(--accent)] text-white flex items-center justify-center text-sm font-medium flex-shrink-0">
                                    {{ $index + 1 }}
                                </div>
                                <div class="flex-1">
