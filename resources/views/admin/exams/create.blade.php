...
@section('admin-content')
    <div>
        <h1 class="text-[22px] font-medium text-[var(--text)] mb-8">Create Exam</h1>
        ...
        <div class="card p-6 max-w-2xl">
            <form method="POST" action="/admin/exams">
                ...
                <div class="mb-4">
                    ...
                    <textarea name="description" rows="3" class="input resize-vertical">{{ old('description') }}</textarea>
                    @error('description')
                        <p class="text-sm text-[var(--danger)] mt-1">{{ $message }}</p>
                    @enderror
                </div>
                ...
                <div class="mb-6">
                    ...
                    <input type="number" name="duration" value="{{ old('duration', 30) }}" class="input" min="1" required>
                    @error('duration')
                        <p class="text-sm text-[var(--danger)] mt-1">{{ $message }}</p>
                    @enderror
                </div>
