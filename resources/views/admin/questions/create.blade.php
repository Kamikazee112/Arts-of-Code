        ...
        <div class="card p-6 max-w-3xl">
            <form method="POST" action="/admin/questions" x-data="{ questionType: '{{ old('type', 'mcq') }}' }">
                ...
                <div class="mb-4">
                    ...
                    <textarea name="question" rows="4" class="input resize-vertical"
                ...
                <div class="mb-4">
                    ...
                    <div class="flex gap-4">
                        <label class="flex items-center">
                            <input type="radio" name="type" value="mcq" {{ old('type', 'mcq') === 'mcq' ? 'checked' : '' }}
                                @input="questionType = 'mcq'" class="mr-2">
                            <span>Multiple Choice</span>
                        </label>
                        <label class="flex items-center">
                            <input type="radio" name="type" value="open" {{ old('type') === 'open' ? 'checked' : '' }}
                                @input="questionType = 'open'" class="mr-2">
                ...
                <div class="mb-4">
                    ...
                    <textarea name="code_snippet" rows="3" class="input resize-vertical font-mono text-sm"
                        placeholder="Paste code here...">{{ old('code_snippet') }}</textarea>
                    @error('code_snippet')
                        <p class="text-sm text-[var(--danger)] mt-1">{{ $message }}</p>
                    @enderror
                </div>
                ...
                <div x-show="questionType === 'open'" class="mb-4">
                    ...
                    <div class="mb-4">
                        ...
                        <textarea name="model_answer" rows="3"
                            class="input resize-vertical">{{ old('model_answer') }}</textarea>
                        @error('model_answer')
                            <p class="text-sm text-[var(--danger)] mt-1">{{ $message }}</p>
                        @enderror
                    </div>
                    ...
                    <div class="mb-4">
                        ...
                        <textarea name="explanation" rows="2"
                            class="input resize-vertical">{{ old('explanation') }}</textarea>
                        @error('explanation')
                            <p class="text-sm text-[var(--danger)] mt-1">{{ $message }}</p>
                        @enderror
                    </div>
                ...
                <div class="mb-6">
                    ...
                    <input type="number" name="points" value="{{ old('points', 1) }}" class="input max-w-xs" min="1"
                        required>
                    @error('points')
                        <p class="text-sm text-[var(--danger)] mt-1">{{ $message }}</p>
                    @enderror
