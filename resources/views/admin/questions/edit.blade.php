...
<div>
    ...
    <div class="card p-6 max-w-3xl">
        <form method="POST" action="/admin/questions/{{ $question->id }}" x-data="{ questionType: '{{ old('type', $question->type) }}' }">
            @csrf
            @method('PATCH')
            ...
            <div class="mb-4">
                ...
                <div class="flex gap-4">
                    <label class="flex items-center">
                        <input type="radio" name="type" value="mcq" {{ old('type', $question->type) === 'mcq' ? 'checked' : '' }} @input="questionType = 'mcq'" class="mr-2">
                        <span>Multiple Choice</span>
                    </label>
                    <label class="flex items-center">
                        <input type="radio" name="type" value="open" {{ old('type', $question->type) === 'open' ? 'checked' : '' }} @input="questionType = 'open'" class="mr-2">
            ...
            <div class="mb-4">
                ...
                >{{ old('code_snippet', $question->code_snippet) }}</textarea>
                @error('code_snippet')
                    <p class="text-sm text-[var(--danger)] mt-1">{{ $message }}</p>
                @enderror
            </div>
            ...
            <div x-show="questionType === 'mcq'" class="mb-4">
                ...
                <div class="space-y-3">
                    ...
                    <div class="flex gap-2 items-center">
                        ...
                        <input type="text" name="option_a" value="{{ old('option_a', $options[0] ?? '') }}" class="input flex-1" required>
                        <input type="radio" name="correct_answer" value="0" {{ old('correct_answer', $question->correct_answer) === '0' ? 'checked' : '' }} class="ml-2">
                    </div>

                    <!-- Option B -->
                    <div class="flex gap-2 items-center">
                        <span class="text-sm font-medium w-8">B:</span>
                        <input type="text" name="option_b" value="{{ old('option_b', $options[1] ?? '') }}" class="input flex-1" required>
                        <input type="radio" name="correct_answer" value="1" {{ old('correct_answer', $question->correct_answer) === '1' ? 'checked' : '' }} class="ml-2">
                    </div>

                    <!-- Option C -->
                    <div class="flex gap-2 items-center">
                        <span class="text-sm font-medium w-8">C:</span>
                        <input type="text" name="option_c" value="{{ old('option_c', $options[2] ?? '') }}" class="input flex-1" required>
                        <input type="radio" name="correct_answer" value="2" {{ old('correct_answer', $question->correct_answer) === '2' ? 'checked' : '' }} class="ml-2">
                    </div>

                    <!-- Option D -->
                    <div class="flex gap-2 items-center">
                        <span class="text-sm font-medium w-8">D:</span>
                        <input type="text" name="option_d" value="{{ old('option_d', $options[3] ?? '') }}" class="input flex-1" required>
                        <input type="radio" name="correct_answer" value="3" {{ old('correct_answer', $question->correct_answer) === '3' ? 'checked' : '' }} class="ml-2">
                    </div>
                </div>
            ...
            <div x-show="questionType === 'open'" class="mb-4">
                ...
                <div class="mb-4">
                    ...
                    <textarea
                        ...
                        rows="3"
                        class="input resize-vertical"
                    >{{ old('model_answer', $question->model_answer) }}</textarea>
                    @error('model_answer')
                        <p class="text-sm text-[var(--danger)] mt-1">{{ $message }}</p>
                    @enderror
                </div>
