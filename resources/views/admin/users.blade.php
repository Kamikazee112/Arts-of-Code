            ...
            <div class="divide-y divide-[var(--border)]">
                @foreach($users as $user)
                    <div class="p-4 flex items-center justify-between hover:bg-[var(--bg)] transition-colors">
                        ...
                        <div class="flex items-center gap-2">
                            @if($user->role === 'user')
                                <form method="POST" action="/admin/users/{{ $user->id }}/promote" onsubmit="return confirm('Are you sure you want to promote {{ $user->username }} to admin?');">
                                    @csrf
                            ...
                            @elseif($user->id !== auth()->id())
                                <form method="POST" action="/admin/users/{{ $user->id }}/demote" onsubmit="return confirm('Are you sure you want to demote {{ $user->username }} from admin?');">
                                    @csrf
