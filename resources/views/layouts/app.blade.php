    ...
    <nav class="sticky top-0 z-50 h-[60px] bg-[var(--surface)]/95 backdrop-blur-sm border-b border-[var(--border)] shadow-[0_1px_8px_rgba(15,23,42,0.06)]"
        ...
        <div class="max-w-[1200px] mx-auto px-6 h-full flex items-center justify-between">
            ...
            <div class="hidden md:flex items-center gap-3">
                @guest
                    ...
                    <a href="/register" class="btn-primary" style="padding:7px 16px;font-size:13px;">إنشاء Account</a>
                @endguest
        ...
        <div x-show="mobileMenuOpen" x-transition class="md:hidden bg-[var(--surface)] border-t border-[var(--border)] py-4 px-6 shadow-md">
            <div class="flex flex-col gap-1">
                <a href="/articles" class="px-3 py-2.5 rounded-lg text-[14px] text-[var(--text)] hover:bg-[var(--bg)] hover:text-[var(--accent)] transition-colors font-medium">المقالات</a>
                @if(auth()->check() && auth()->user()->role === 'admin')
                    <a href="/questions" class="px-3 py-2.5 rounded-lg text-[14px] text-[var(--text)] hover:bg-[var(--bg)] hover:text-[var(--accent)] transition-colors font-medium">الأسئلة</a>
                @endif
                ...
                @guest
                    ...
                    <a href="/register" class="btn-primary text-center mt-1">إنشاء Account</a>
                @endguest
                @auth
                    ...
                    <a href="/settings" class="px-3 py-2.5 rounded-lg text-[14px] text-[var(--text)] hover:bg-[var(--bg)] font-medium">الإعدادات</a>
                    <form method="POST" action="/logout">
                        @csrf
