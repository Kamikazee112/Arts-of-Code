...
@section('content')
    <div class="max-w-[800px] mx-auto">
        <!-- Page Title -->
        <div class="mb-8">
            <h1 class="text-[22px] font-medium text-[var(--text)] mb-2">Leaderboard</h1>
        ...
        @if(isset($users) && $users->count() > 0)
            <div>
                ...
                @foreach($users as $user)
                    <div
                        class="flex items-center px-4 py-4 border-b border-[var(--border)] {{ auth()->check() && auth()->id() === $user->id ? 'border-l-3 border-l-[var(--accent)] bg-[#EFF6FF]' : '' }}">
                        <!-- Rank -->
                        ...
                        <div class="flex-1">
                            <a href="/users/{{ $user->username }}"
                                class="font-medium text-[var(--text)] hover:text-[var(--accent)] transition-colors">
                                {{ $user->username }}
                            </a>
