...
<div class="fixed bottom-6 right-6 z-[9999] flex flex-col gap-2" x-data="{ show: true }">
    <!-- Flash Messages -->
    @if(session('success'))
        <div class="bg-white border border-[var(--border)] rounded-lg p-4 min-w-[280px] max-w-[360px] shadow-lg"
            style="border-left: 3px solid #16A34A" x-data="{ show: true }" x-show="show" x-transition
    ...
    @if(session('error'))
        <div class="bg-white border border-[var(--border)] rounded-lg p-4 min-w-[280px] max-w-[360px] shadow-lg"
            style="border-left: 3px solid var(--danger)" x-data="{ show: true }" x-show="show" x-transition
            x-init="setTimeout(() => show = false, 4000)">
            <div class="flex justify-between items-start">
    ...
    @if(session('info'))
        <div class="bg-white border border-[var(--border)] rounded-lg p-4 min-w-[280px] max-w-[360px] shadow-lg"
            style="border-left: 3px solid var(--accent)" x-data="{ show: true }" x-show="show" x-transition
            x-init="setTimeout(() => show = false, 4000)">
            <div class="flex justify-between items-start">
